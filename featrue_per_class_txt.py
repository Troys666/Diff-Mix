...
def precompute_text_clip_features(model, train_dataset, device, weight_dtype):
    """预先计算每个类别的文本嵌入特征"""
    print("正在预先计算文本嵌入特征...")
    
    # 获取CLIP文本模型
    clip_text_model = model.pipe.text_encoder
    clip_tokenizer = model.pipe.tokenizer
    
    # 为每个类别计算文本特征
    class_text_features = {}
    
    for class_id in range(len(train_dataset.class_names)):
        # 获取该类别的一个样本元数据来构建prompt
        label_indices = train_dataset.label_to_indices[class_id]
        ...
        # 使用text_encoder获取完整的文本嵌入（用于UNet）
        ...
        with torch.no_grad():
            # 获取完整的文本编码器输出（用于UNet）
            text_encoder_hidden_states = clip_text_model(text_input_ids)[0].to(dtype=weight_dtype)
            # 全局平均池化得到CLIP文本特征（用于CLIP损失）
            text_clip_features = text_encoder_hidden_states.mean(dim=1)  # [1, D]
        
        class_text_features[class_id] = {
            "encoder_hidden_states": text_encoder_hidden_states.detach().cpu(),  # 完整编码器输出
            "clip_features": text_clip_features.detach().cpu()  # CLIP特征
        }
        print(f"类别 {class_id}: {prompt} -> 编码器输出维度: {text_encoder_hidden_states.shape}, CLIP特征维度: {text_clip_features.shape}")
...
def dataset_feature_per_class(dataset, num_classes):
    ...
    # 预先计算文本CLIP特征
    class_text_features = precompute_text_clip_features(model, train_dataset, device, weight_dtype)
    ...
    for batch in tqdm(train_dataloader, desc="Processing batches"):
        ...
        for label in labels:
            ...
            text_clip_features.append(class_text_features[class_id]["clip_features"])
            encoder_hidden_states_list.append(class_text_features[class_id]["encoder_hidden_states"])
        text_clip_features = torch.cat(text_clip_features, dim=0)  # [batch_size, D]
        encoder_hidden_states = torch.cat(encoder_hidden_states_list, dim=0)  # [batch_size, seq_len, D]
        ...
        for i in range(bsz):
            ...
            text_clip_feat = text_clip_features[i:i+1]  # [1, D] - 新增文本CLIP特征
            ...
            sums_text_clip[label] = (
                text_clip_feat.clone() if sums_text_clip[label] is None else sums_text_clip[label] + text_clip_feat
            )
    ...
    # 保存均值：将本次运行包裹在一个大的文件夹中（包含文本CLIP特征）
    ...
    for cls_id in range(num_classes):
        ...
        avg_text_clip = None if sums_text_clip[cls_id] is None else sums_text_clip[cls_id] / cnt  # 新增文本CLIP特征
        ...
        save_obj = {
            ...
            "text_clip_features": avg_text_clip,  # 文本的CLIP特征（用于CLIP损失）
            "text_encoder_hidden_states": class_text_features[cls_id]["encoder_hidden_states"],  # 完整的文本编码器输出（用于UNet）
